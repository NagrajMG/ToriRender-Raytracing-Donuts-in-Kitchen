    ...
    iter_impl  // NOLINT(cppcoreguidelines-special-member-functions,hicpp-special-member-functions)
{
  ...
  using other_iter_impl =
      iter_impl<typename std::conditional<std::is_const<BasicJsonType>::value,
  ...
  */
  ...
  @note It is not checked whether @a other is initialized.
  */
  iter_impl& operator=(const iter_impl<typename std::remove_const<BasicJsonType>::type>&
  ...
  */
  ...
  @pre (1) Both iterators are initialized to point to the same object, or (2) both iterators are
  value-initialized.
  */
  ...
  bool operator==(const IterImpl& other) const {
    ...
    // value-initialized forward iterators can be compared, and must compare equal to other
    // value-initialized iterators of the same type #4493
    if (m_object == nullptr) {
      return true;
    }
  ...
  */
  bool operator<(const iter_impl& other) const {
    ...
    // value-initialized iterators of the same type #4493
    if (m_object == nullptr) {
      // the iterators are both value-initialized and are to be considered equal, but this function
      // checks for smaller, so we return false
      return false;
    }
  ...
  */
  ...
  @pre (1) The iterator is initialized; i.e. `m_object != nullptr`, or (2) both iterators are
  value-initialized.
  */
  bool operator>=(const iter_impl& other) const {
    return !operator<(other);
