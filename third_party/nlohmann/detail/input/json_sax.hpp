...
class json_sax_dom_parser {
 ...
 private:
  ...
  void handle_diagnostic_positions_for_json_value(BasicJsonType& v) {
    if (m_lexer_ref) {
      // Lexer has read past the current field value, so set the end position to the current
      // position. The start position will be set below based on the length of the string
...
class json_sax_dom_callback_parser {
 public:
  ...
  bool start_object(std::size_t len) {
    ...
    if (ref_stack.back()) {
      ...
      // check object limit
      if (JSON_HEDLEY_UNLIKELY(len != detail::unknown_size() &&
  ...
  */
  ...
  std::pair<bool, BasicJsonType*> handle_value(Value&& v, const bool skip_callback = false) {
    ...
    // check callback
    const bool keep =
        skip_callback || callback(static_cast<int>(ref_stack.size()), parse_event_t::value, value);
