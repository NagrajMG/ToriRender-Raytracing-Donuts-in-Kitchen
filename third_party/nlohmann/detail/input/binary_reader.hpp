...
class binary_reader {
  ...
  */
  bool parse_bson_element_internal(const char_int_type element_type,
    ...
    switch (element_type) {
      ...
      {
        std::uint64_t value{};
        return get_number<std::uint64_t, true>(input_format_t::bson, value) &&
  ...
  bool parse_cbor_internal(const bool get_char, const cbor_tag_handler_t tag_handler) {
    switch (get_char ? get() : current) {
      ...
      case 0xF9:  // Half-Precision Float (two-byte IEEE 754)
      {
        ...
        return sax->number_float((half & 0x8000u) != 0 ? static_cast<number_float_t>(-val)
        ...
        return sax->parse_error(
            ...
            parse_error::create(
                ...
                exception_message(input_format_t::cbor,
                                  concat("expected length specification (0x60-0x7B) or indefinite "
      ...
      default: {
        ...
        return sax->parse_error(
            ...
            parse_error::create(
                ...
                chars_read,
                exception_message(input_format_t::cbor,
                                  concat("expected length specification (0x40-0x5B) or indefinite "
                                         "binary array type (0x5F); last byte: 0x",
                                         last_token),
                                  "binary"),
                nullptr));
      }
      ...
      default:  // anything else
      {
        ...
        return sax->parse_error(
            ...
            parse_error::create(
                ...
                chars_read,
                exception_message(
                    input_format_t::msgpack, concat("invalid byte: 0x", last_token), "value"),
                nullptr));
      }
    }
  }
  ...
  */
  bool get_msgpack_string(string_t& result) {
    ...
    switch (current) {
      ...
      case 0xBF: {
        return get_string(
            input_format_t::msgpack, static_cast<unsigned int>(current) & 0x1Fu, result);
      }
  ...
  */
  bool get_ubjson_string(string_t& result, const bool get_char = true) {
    ...
    return sax->parse_error(
        ...
        last_token,
        parse_error::create(
            113, chars_read, exception_message(input_format, message, "string"), nullptr));
  }
  ...
  bool get_ubjson_size_value(std::size_t& result, bool& is_ndarray, char_int_type prefix = 0) {
    ...
    switch (prefix) {
        ...
        if (number < 0) {
          return sax->parse_error(
              ...
              parse_error::create(
                  ...
                  chars_read,
                  exception_message(
                      input_format, "count in an optimized container must be positive", "size"),
                  nullptr));
        }
      ...
      case '[': {
        ...
        if (is_ndarray)  // ndarray dimensional vector can only contain integers, and can not embed
                         // another array
        {
          return sax->parse_error(
              chars_read,
              ...
              parse_error::create(
                  ...
                  chars_read,
                  exception_message(
                      input_format, "ndarray dimensional vector is not allowed", "size"),
                  nullptr));
        }
        ...
        if (!dim.empty())  // if ndarray, convert to an object in JData annotated array format
        {
          ...
          for (auto i : dim) {
            result *= i;
            if (result == 0 ||
                result == npos)  // because dim elements shall not have zeros, result = 0 means
                                 // overflow happened; it also can't be npos as it is used to
                                 // initialize size in get_ubjson_size_type()
            {
              return sax->parse_error(
                  chars_read,
                  get_token_string(),
    ...
    if (input_format != input_format_t::bjdata) {
      ...
      message =
          "expected length type specification (U, i, u, I, m, l, M, L) after '#'; last byte: 0x" +
          last_token;
    }
    return sax->parse_error(
        chars_read,
        last_token,
        parse_error::create(
            113, chars_read, exception_message(input_format, message, "size"), nullptr));
  }
  ...
  bool get_ubjson_size_type(std::pair<std::size_t, char_int_type>& result,
    ...
    if (current == '$') {
      ...
      if (input_format == input_format_t::bjdata && is_ndarray) {
        ...
        result.second |= (1 << 8);  // use bit 8 to indicate ndarray, all UBJSON and BJData markers
                                    // should be ASCII letters
      }
      return is_error;
    }
  ...
  bool get_ubjson_value(const char_int_type prefix) {
        ...
        if (JSON_HEDLEY_UNLIKELY(current > 127)) {
          ...
          return sax->parse_error(
              ...
              parse_error::create(
                  ...
                  exception_message(
                      input_format,
                      concat("byte after 'C' must be in range 0x00..0x7F; last byte: 0x",
                             last_token),
                      "char"),
                  nullptr));
        }
    ...
    return sax->parse_error(
        ...
        parse_error::create(
            112,
            chars_read,
            exception_message(input_format, "invalid byte: 0x" + last_token, "value"),
            nullptr));
  }
  ...
  */
  bool get_ubjson_array() {
    ...
    // if bit-8 of size_and_type.second is set to 1, encode bjdata ndarray as an object in JData
    // annotated array format (https://github.com/NeuroJSON/jdata):
    // {"_ArrayType_" : "typeid", "_ArraySize_" : [n1, n2, ...], "_ArrayData_" : [v1, v2, ...]}

    if (input_format == input_format_t::bjdata && size_and_type.first != npos &&
        (size_and_type.second & (1 << 8)) != 0) {
      size_and_type.second &=
          ~(static_cast<char_int_type>(1) << 8);  // use bit 8 to indicate ndarray, here we remove
                                                  // the bit to restore the type marker
  ...
  */
  ...
  bool get_ubjson_high_precision_number() {
    ...
    auto number_lexer =
        detail::lexer<BasicJsonType, ia_type>(detail::input_adapter(number_vector), false);
...
#define JSON_BINARY_READER_MAKE_BJD_TYPES_MAP_  \
  make_array<bjd_type>(bjd_type{'B', "byte"},   \
                       bjd_type{'C', "char"},   \
                       bjd_type{'D', "double"}, \
                       bjd_type{'I', "int16"},  \
                       bjd_type{'L', "int64"},  \
  ...
  JSON_PRIVATE_UNLESS_TESTED :
      ...
      const decltype(JSON_BINARY_READER_MAKE_BJD_OPTIMIZED_TYPE_MARKERS_)
          bjd_optimized_type_markers = JSON_BINARY_READER_MAKE_BJD_OPTIMIZED_TYPE_MARKERS_;
