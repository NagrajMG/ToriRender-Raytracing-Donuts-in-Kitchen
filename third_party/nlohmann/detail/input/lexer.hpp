...
class lexer : public lexer_base<BasicJsonType> {
          ...
          switch (get()) {
            ...
            case 'u': {
              ...
              if (codepoint < 0x80) {
                ...
                add(static_cast<char_int_type>(0xC0u |
                                               (static_cast<unsigned int>(codepoint) >> 6u)));
                add(static_cast<char_int_type>(0x80u |
                                               (static_cast<unsigned int>(codepoint) & 0x3Fu)));
              } else if (codepoint <= 0xFFFF) {
                // 3-byte characters: 1110xxxx 10xxxxxx 10xxxxxx
                add(static_cast<char_int_type>(0xE0u |
                ...
                add(static_cast<char_int_type>(0xF0u |
  ...
  */
  token_type
  scan_number()  // lgtm [cpp/use-of-goto] `goto` is used in this function to implement the
                 // number-parsing state machine described above. By design, any finite input will
                 // eventually reach the "done" state or return token_type::parse_error. In each
                 // intermediate state, 1 byte of the input is appended to the token_buffer vector,
                 // and only the already initialized variables token_buffer, number_type, and
                 // error_message are manipulated.
  {
  ...
  scan_number_done:
    ...
    char* endptr =
        nullptr;  // NOLINT(misc-const-correctness,cppcoreguidelines-pro-type-vararg,hicpp-vararg)
    errno = 0;
