...
class parser {
  ...
  */
  void parse(const bool strict, BasicJsonType& result) {
    if (callback) {
      json_sax_dom_callback_parser<BasicJsonType, InputAdapterType> sdp(
          result, callback, allow_exceptions, &m_lexer);
 ...
 private:
  ...
  std::string exception_message(const token_type expected, const std::string& context) {
    ...
    if (last_token == token_type::parse_error) {
      error_msg +=
          concat(m_lexer.get_error_message(), "; last read: '", m_lexer.get_token_string(), '\'');
    } else {
