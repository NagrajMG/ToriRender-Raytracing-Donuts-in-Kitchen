 ...
 public:
  ...
  std::size_t get_elements(T* dest, std::size_t count = 1) {
    auto res = static_cast<std::size_t>(
        sb->sgetn(reinterpret_cast<char*>(dest), static_cast<std::streamsize>(count * sizeof(T))));
...
class iterator_input_adapter {
 public:
  ...
  // for general iterators, we cannot really do something better than falling back to processing the
  // range one-by-one
  template <class T>
  std::size_t get_elements(T* dest, std::size_t count = 1) {
    ...
    if (JSON_HEDLEY_UNLIKELY(input.empty())) {
      ...
      if (wc < 0x80) {
        ...
        if (JSON_HEDLEY_UNLIKELY(!input.empty())) {
          ...
          utf8_bytes[1] =
              static_cast<std::char_traits<char>::int_type>(0x80u | ((charcode >> 12u) & 0x3Fu));
          utf8_bytes[2] =
              static_cast<std::char_traits<char>::int_type>(0x80u | ((charcode >> 6u) & 0x3Fu));
...
class wide_string_input_adapter {
 public:
  ...
  // parsing binary with wchar doesn't make sense, but since the parsing mode can be runtime, we
  // need something here
  template <class T>
  std::size_t get_elements(T* /*dest*/, std::size_t /*count*/ = 1) {
    JSON_THROW(parse_error::create(
        112, 1, "wide string type cannot be interpreted as binary data", nullptr));
  }
...
inline file_input_adapter input_adapter(std::FILE* file) {
  if (file == nullptr) {
    JSON_THROW(parse_error::create(101,
                                   0,
...
contiguous_bytes_input_adapter input_adapter(CharT b) {
  ...
  return input_adapter(ptr,
                       ptr + length);  // cppcheck-suppress[nullPointerArithmeticRedundantCheck]
}
