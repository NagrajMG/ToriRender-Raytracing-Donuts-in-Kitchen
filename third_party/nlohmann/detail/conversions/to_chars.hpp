...
struct diyfp  // f * 2^e
{
  ...
  */
  static diyfp mul(const diyfp& x, const diyfp& y) noexcept {
    ...
    //   = (u_lo v_lo         ) + 2^32 ((u_lo v_hi         ) + (u_hi v_lo         )) + 2^64 (u_hi
    //   v_hi         ) = (p0                ) + 2^32 ((p1                ) + (p2                ))
    //   + 2^64 (p3                ) = (p0_lo + 2^32 p0_hi) + 2^32 ((p1_lo + 2^32 p1_hi) + (p2_lo +
    //   2^32 p2_hi)) + 2^64 (p3                ) = (p0_lo             ) + 2^32 (p0_hi + p1_lo +
    //   p2_lo                      ) + 2^64 (p1_hi + p2_hi + p3) = (p0_lo             ) + 2^32 (Q
    //   ) + 2^64 (H                 ) = (p0_lo             ) + 2^32 (Q_lo + 2^32 Q_hi ) + 2^64 (H )
    //
...
boundaries compute_boundaries(FloatType value) {
  ...
  const diyfp v =
      is_denormal ? diyfp(F, kMinExp) : diyfp(F + kHiddenBit, static_cast<int>(E) - kBias);
...
inline void grisu2_digit_gen(
  ...
  std::uint64_t dist =
      diyfp::sub(M_plus, w).f;  // (significand of (M+ - w ), implicit exponent is e)
  ...
  auto p1 = static_cast<std::uint32_t>(
      M_plus.f >> -one.e);  // p1 = f div 2^-e (Since -e >= 32, p1 fits into a 32-bit int.)
