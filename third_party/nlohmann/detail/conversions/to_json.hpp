...
inline void to_json(BasicJsonType& j, EnumType e) noexcept {
  ...
  static constexpr value_t integral_value_t =
      std::is_unsigned<underlying_type>::value ? value_t::number_unsigned : value_t::number_integer;
...
template <
    typename BasicJsonType,
    typename T,
    enable_if_t<std::is_same<T, iteration_proxy_value<typename BasicJsonType::iterator>>::value,
                int> = 0>
...
inline void to_json_tuple_impl(BasicJsonType& j,
                               const Tuple& t,
...
inline void to_json(BasicJsonType& j, const std::basic_string<char8_t, Tr, Allocator>& s) {
  ...
  j = std::basic_string<char, std::char_traits<char>, OtherAllocator>(
      s.begin(), s.end(), s.get_allocator());
}
#endif
