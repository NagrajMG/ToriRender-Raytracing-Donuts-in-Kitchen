...
// std::pair and std::tuple until LWG 2367 fix (see
// https://cplusplus.github.io/LWG/lwg-defects.html#2367). This causes compile errors in e.g.
// clang 3.5 or gcc 4.9.
template <typename T>
...
struct is_range {
 private:
  ...
  // https://en.cppreference.com/w/cpp/iterator/sentinel_for but reimplementing these would be too
...
struct is_compatible_object_type_impl<
  ...
  static constexpr bool value = is_constructible<typename object_t::key_type,
...
struct is_constructible_object_type_impl<
  ...
  static constexpr bool value =
      ...
      (has_from_json<BasicJsonType, typename ConstructibleObjectType::mapped_type>::value ||
       has_non_default_from_json<BasicJsonType,
                                 typename ConstructibleObjectType::mapped_type>::value);
};
...
struct is_constructible_string_type {
  ...
  static constexpr auto value =
      conjunction<is_constructible<laundered_type, typename BasicJsonType::string_t>,
...
struct is_compatible_array_type_impl<
    ...
    enable_if_t<
        ...
        // special case for types like std::filesystem::path whose iterator's value_type are
        // themselves c.f. https://github.com/nlohmann/json/pull/3073
...
struct is_constructible_array_type_impl<
    ...
    enable_if_t<!std::is_same<ConstructibleArrayType, typename BasicJsonType::value_type>::value &&
                ...
                is_iterator_traits<
                    iterator_traits<detected_t<iterator_t, ConstructibleArrayType>>>::value &&
  ...
  static constexpr bool value =
      std::is_same<value_type, typename BasicJsonType::array_t::value_type>::value ||
...
struct is_compatible_type_impl<BasicJsonType,
                               CompatibleType,
...
using has_erase_with_key_type = typename std::conditional<
    is_detected<detect_erase_with_key_type, typename BasicJsonType::object_t, KeyType>::value,
    std::true_type,
    std::false_type>::type;
...
template <typename... Types>
using same_sign =
    std::integral_constant<bool, all_signed<Types...>::value || all_unsigned<Types...>::value>;
...
struct value_in_range_of_impl2<OfType, T, false, true> {
  static constexpr bool test(T val) {
    using CommonType = typename std::common_type<OfType, T>::type;
    return val >= 0 && static_cast<CommonType>(val) <=
                           static_cast<CommonType>((std::numeric_limits<OfType>::max)());
  }
};
